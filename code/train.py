...
def load_graph_data(grath_path):
    """
    Load the pre-processed graph data and mappings.
    - grath_path: Directory containing graph_data.pt and acct_mappings.csv
    """
...
def TrainModel(model, data, model_save_path, fold_num=0, epochs=200, lr=0.001, weight_decay=5e-4): # <-- (*** tqdm added ***) Added fold_num
    """
    (*** K-Fold Modified Version ***)
    Train the GNN Model
    - data object must contain data.train_mask and data.val_mask
    - model_save_path is the path to save the best model for this fold
    """
    # GPU setup
