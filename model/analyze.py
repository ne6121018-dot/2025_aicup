"""
Model Analysis & Interpretability Module
----------------------------------------
本模組使用 GNNExplainer 對訓練好的 FraudGNN 模型進行可解釋性分析 (Interpretability)。

主要功能：
1. 載入資料：讀取圖資料、特徵名稱與預測結果。
2. 載入模型：讀取 Fold 1 的最佳模型權重作為代表。
3. GNNExplainer：針對被預測為詐欺 (Label=1) 的節點進行解釋，計算特徵重要性分數。
4. 視覺化：繪製 Global Feature Importance 長條圖並儲存為 CSV 與 PNG。
...
def PlotFeatureImportance(importance_df, save_path):
    ...
    Args:
        importance_df (pd.DataFrame): 包含 'feature' 與 'importance' 的資料表。
        save_path (str): 圖片儲存路徑 (.png)。
    """
...
def load_analysis_data(kfold_dir):
    ...
    Args:
        root_dir (str): 專案根目錄 (包含 graph/, feature/ 等)。
        result_dir (str): 存放預測結果 CSV 的目錄。

    Returns:
        tuple: (data, acct_to_idx, feature_cols, df_pred_ensembled)
    """
