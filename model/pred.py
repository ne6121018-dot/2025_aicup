...
-----------------------------------
本模組負責載入訓練好的 FraudGNN 模型進行推論 (Inference)。

主要功能：
1. K-Fold Ensemble：載入 5 個 Fold 訓練出的最佳模型權重。
2. 模型推論：對測試集 (acct_predict.csv) 進行預測。
3. 結果集成：將 5 個模型的預測機率取平均 (Average Ensemble)。
4. 閾值切分：根據設定的 Threshold (預設 0.85) 將機率轉換為 0/1 標籤。
5. 輸出結果：生成符合比賽格式的提交檔案 (result_gnn_ensembled.csv)。
...
def load_graph_data(processed_dir, raw_data_dir):
    ...
    Args:
        processed_dir (str): 包含 graph_data.pt 與 acct_mappings.csv 的目錄 (通常是 graph/)。
        raw_data_dir (str): 包含原始 acct_predict.csv 的目錄 (通常是 Dataset/)。

    Returns:
        tuple: (data, acct_to_idx, idx_to_acct, df_test)
    """
...
def Predict(model, data, idx_to_acct):
    ...
    Args:
        model (FraudGNN): 載入權重的模型實例。
        data (Data): 圖資料物件。
        idx_to_acct (dict): 索引轉帳號的字典。

    Returns:
        pd.DataFrame: 包含 'acct' 與 'probability' 的預測結果。
    """
...
def OutputCSV(path, df_test, df_pred_ensembled):
    ...
    Args:
        path (str): 輸出檔案路徑。
        df_test (pd.DataFrame): 原始測試集名單 (確保順序與數量一致)。
        df_pred_ensembled (pd.DataFrame): 集成後的預測結果。
    """
